...
from SCRIPTS.functions.cls_DataBases import fnc_connect_local, fnc_close_local, fnc_execute_sql
...
def prc_executa_db(sql_query, params):
    ...
    try:
        ...
        log_info = "F3"
        fnc_execute_sql(conexao, sql_query, params)
...
def main():
    ...
    try:
        ...
        if emails_data:
                ...
                if email_data['assunto'][:5].upper() in ["RES: ", "ENC: "]:
                    ...
                    if processed_email["email"].startswith('SPAM,'):
                        ...
                        if processed_email["telefone"] != "Não identificado":
                            resultado = enviar_whatsapp_anexo("PSM - ADMINISTRAÇÃO", processed_email["mensagem"],
                                                              file_name)
            ...
            """
            params = {
                "nome": processed_email["nome"],
                "telefone": processed_email["telefone"],
                "email": processed_email["email"],
                "assunto": email_data["assunto"],
                "mensagem": processed_email["mensagem"],
                "c_anexos": processed_email["c_anexos"],
                "tag": mail_tag
            }
